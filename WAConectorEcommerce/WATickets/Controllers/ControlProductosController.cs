    ...
    public class ControlProductosController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Get([FromUri] Filtros filtro)
        {
            try
            {
                ...
                var Bitacoras = db.BitacoraMovimientos.Where(a => a.idEncabezado == filtro.Codigo1 && a.ProcesadaSAP == true).ToList(); //Hago el llamado de las bitacoras de movimiento que tengan el id del encabezado de repracion
                ...
                foreach (var item in bitacorasEntradas)
                {
...

                    {
                        ...
                        if (Item == null) //Si no existe el articulo en el detalle
                        {
                            ...
                            detMovimiento.CantidadUsado = item2.Cantidad;
                        ...
                        {
                            control.Remove(Item); 
                            Item.CantidadUsado += item2.Cantidad;
                            control.Add(Item);
