                    ...
                    else
                    {
                        ...
                        if (respuesta == 0)
                        {
                            ...
                            try
                            {
                                ...
                                bodyH = bodyH.Replace("@CelularCliente", "      ");
                                bodyH = bodyH.Replace("@DocEntry", EncMovimiento.NumLlamada);
                                ...
                                bodyH = bodyH.Replace("@TotalGeneral", EncMovimiento.TotalComprobante.ToString());
                                ...
                                foreach (var item in DetalleSAP)
                                {
                                    ...
                                    if (z == 0)
                                    {
                                         inyectado = Html.Inyectado.Replace("@ItemCode", item.ItemCode).Replace("@ItemName", item.ItemName).Replace("@Cantidad", item.Cantidad.ToString()).Replace("@PrecioUnitario", item.PrecioUnitario.ToString()).Replace("@TotalLinea", item.TotalLinea.ToString());
